...
private:
	...
	void Release_node(void(*Visit)(BinaryTreeNode<T>* u,T & item), BinaryTreeNode<T> * t,T & item);
public:
	...
	bool release_node(T & item)
	{
		...
		Release_node(Find_node, root, item);
		return true;
	}
};
...
template<class T>
inline void BinaryTree<T>::Release_node(BinaryTreeNode<T>* t, T & item)
{
	{
		cout << "hahah\n";
		if (t)
...
template<class T>
inline void BinaryTree<T>::Release_node(void(*Visit)(BinaryTreeNode<T>*u, T &item), BinaryTreeNode<T> * t, T & item)
{
	if (t)
	{
		Release_node(t->LeftChild,item);
